	...
	while true do
		...
		if showHelp then
			if helpTimer > GetGameTimer() then
				showHelpText("Navigué avec ~y~Z,Q,S,D~s~ et confirmé avec ~y~ESPACE~s~ pour le bloc de code gauche.")
			elseif helpTimer > GetGameTimer()-helpCycle then
				showHelpText("Utilisez les ~y~Flèches Directionnelle~s~ et ~y~ENTRER~s~ pour le bloc de code droit")
			else
